        ...
        mongoose.connect(`mongodb+srv://Ankan157:Ankan1567@resultshowdatabse.q1ops.mongodb.net/${CourseName}?retryWrites=true&w=majority`).then(()=>{
            ...
            if(Semester == '1st Semester'){
                if(FindMethod == 'RegistrationNumber'){
                    ...
                    MongoModel.FirstSemModel.find({"Column3":SearchValue}, (error, data)=>{
                        if(error){
                            responses.status(404).json({status:'Unable To Get Your Result Right Now'})
                    ...
                    MongoModel.FirstSemModel.find({"Column1":SearchValue}, (error, data)=>{
                        if(error){
                            responses.status(404).json({status:'Unable To Get Your Result Right Now'})
                            mongoose.connection.close()
                        }
            ...
            // BA Second Semester
                ...
                if(FindMethod == 'RegistrationNumber'){
                    ...
                    MongoModel.SecondSemModel.find({"Column3":SearchValue}, (error, data)=>{
                        if(error){
                            responses.status(404).json({status:'Unable To Get Your Result Right Now'})
                            mongoose.connection.close()
                        }
                    ...
                    MongoModel.SecondSemModel.find({"Column1":SearchValue}, (error, data)=>{
                        if(error){
                            responses.status(404).json({status:'Unable To Get Your Result Right Now'})
                            mongoose.connection.close()
                        }
            ...
            // BA 3rd Semester
                ...
                if(FindMethod == 'RegistrationNumber'){
                    ...
                    MongoModel.ThirdSemModel.find({"Column3":SearchValue}, (error, data)=>{
                        if(error){
                            responses.status(404).json({status:'Unable To Get Your Result Right Now'})
                            mongoose.connection.close()
                        }
                    ...
                    MongoModel.ThirdSemModel.find({"Column1":SearchValue}, (error, data)=>{
                        if(error){
                            responses.status(404).json({status:'Unable To Get Your Result Right Now'})
                            mongoose.connection.close()
                        }
            ...
            // BA 4th Semester
                ...
                if(FindMethod == 'RegistrationNumber'){
                    ...
                    MongoModel.FourthSemModel.find({"Column3":SearchValue}, (error, data)=>{
                        if(error){
                            responses.status(404).json({status:'Unable To Get Your Result Right Now'})
                            mongoose.connection.close()
                        }
                    ...
                    MongoModel.FourthSemModel.find({"Column1":SearchValue}, (error, data)=>{
                        if(error){
                            responses.status(404).json({status:'Unable To Get Your Result Right Now'})
                            mongoose.connection.close()
                        }
            ...
            // BA Fifth Semester
                ...
                if(FindMethod == 'RegistrationNumber'){
                    ...
                    MongoModel.FifthSemModel.find({"Column3":SearchValue}, (error, data)=>{
                        if(error){
                            responses.status(404).json({status:'Unable To Get Your Result Right Now'})
                            mongoose.connection.close()
                        }
                    ...
                    MongoModel.FifthSemModel.find({"Column1":SearchValue}, (error, data)=>{
                        if(error){
                            responses.status(404).json({status:'Unable To Get Your Result Right Now'})
                            mongoose.connection.close()
                        }
            ...
            // BA Sixth Semester
                ...
                if(FindMethod == 'RegistrationNumber'){
                    ...
                    MongoModel.SixthSemModel.find({"Column3":SearchValue}, (error, data)=>{
                        if(error){
                            responses.status(404).json({status:'Unable To Get Your Result Right Now'})
                            mongoose.connection.close()
                        }
                    ...
                    MongoModel.SixthSemModel.find({"Column1":SearchValue}, (error, data)=>{
                        if(error){
                            responses.status(404).json({status:'Unable To Get Your Result Right Now'})
                            mongoose.connection.close()
                        }
        ...
        mongoose.connect(`mongodb+srv://Ankan157:Ankan1567@resultshowdatabse.q1ops.mongodb.net/${CourseName}?retryWrites=true&w=majority`).then(()=>{
            ...
            if(Semester == '1st Semester'){
                if(FindMethod == 'RegistrationNumber'){
                    MongoModel.FirstSemModel.find({"Column3":SearchValue}, (error, data)=>{
                        if(error){
                            responses.status(502).json({status:'Unable To Get Your Result Right Now'})
                            mongoose.connection.close()
                        }
                        else if(data){
                            console.log(data)
